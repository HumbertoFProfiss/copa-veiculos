<div>
    @if ($enviado)
        <div class="max-w-lg mx-auto flex flex-col items-center text-center gap-3 p-8 rounded-2xl bg-white dark:bg-duskcard border border-brand-100 dark:border-duskborder shadow-brand">
            <x-heroicon-o-check-circle class="w-10 h-10 text-brand-700" />
            <h3 class="font-heading text-lg font-semibold text-brand-900 dark:text-white">Proposta recebida!</h3>
            <p class="text-sm text-muted dark:text-duskmuted">Recebemos as informações do seu veículo e entraremos em contato em breve. Ou fale agora pelo WhatsApp!</p>
    ...
    @else
        <form wire:submit="enviar" class="max-w-3xl mx-auto rounded-2xl border border-brand-100 dark:border-duskborder bg-white dark:bg-duskcard p-6 sm:p-8 shadow-brand">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs text-muted dark:text-duskmuted mb-1">Marca do veículo</label>
                    <input type="text" wire:model="marca" placeholder="Ex: Toyota" class="input">
                </div>
                <div>
                    <label class="block text-xs text-muted dark:text-duskmuted mb-1">Modelo</label>
                    <input type="text" wire:model="modelo" placeholder="Ex: Corolla" class="input">
                </div>
                <div>
                    <label class="block text-xs text-muted dark:text-duskmuted mb-1">Ano</label>
                    <input type="text" wire:model="ano" placeholder="2020" class="input">
                </div>
                <div>
                    <label class="block text-xs text-muted dark:text-duskmuted mb-1">Quilometragem</label>
                    <input type="text" wire:model="km" placeholder="Ex: 50.000 km" class="input">
                </div>
                <div>
                    <label class="block text-xs text-muted dark:text-duskmuted mb-1">Valor pretendido (R$)</label>
                    <input type="text" wire:model="valorPretendido" placeholder="Ex: R$ 45.000" class="input">
                </div>
                <div class="sm:col-span-2">
                    <label class="block text-xs text-muted dark:text-duskmuted mb-1">Observações</label>
            ...
            <div class="text-center mt-6">
                ...
                <p class="text-xs text-muted dark:text-duskmuted mt-3">* Campos obrigatórios. Seus dados são usados apenas para contato.</p>
            </div>
        </form>
    @endif
</div>
