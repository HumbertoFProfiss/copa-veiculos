<div>
    ...
    <h1 class="font-heading text-2xl font-semibold text-brand-900 mb-1">Favoritos</h1>
    <p class="text-sm text-muted mb-8">Veículos que você salvou</p>
    ...
    @if ($favoritos->isEmpty())
        <div class="rounded-2xl border border-brand-100 bg-white p-10 text-center">
            ...
            <p class="text-muted">Você ainda não favoritou nenhum veículo.</p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            @foreach ($favoritos as $favorito)
                ...
                <div class="rounded-2xl border border-brand-100 bg-white overflow-hidden">
                    <a href="{{ route('veiculo.show', $veiculo) }}" class="block aspect-[4/3] bg-brand-100/50">
                    ...
                    <div class="p-5">
                        <a href="{{ route('veiculo.show', $veiculo) }}" class="font-heading font-medium text-brand-900 text-sm hover:text-brand-700">{{ $veiculo->marca }} {{ $veiculo->modelo }}</a>
                        ...
                        <button wire:click="remover({{ $favorito->id }})" wire:confirm="Remover este veículo dos favoritos?"
                                class="mt-3 text-xs text-muted hover:text-red-500 flex items-center gap-1">
