<div>
    ...
    <h1 class="font-heading text-2xl font-semibold text-brand-900 mb-1">Garantias</h1>
    <p class="text-sm text-muted mb-8">Status da garantia dos seus veículos</p>
    ...
    @if ($compras->isEmpty())
        <div class="rounded-2xl border border-brand-100 bg-white p-10 text-center">
            ...
            <p class="text-muted">Nenhuma garantia ativa. Elas aparecem aqui após a confirmação da compra.</p>
        </div>
    @else
        <div class="space-y-5">
            @foreach ($compras as $venda)
                <div class="rounded-2xl border border-brand-100 bg-white overflow-hidden">
                    <div class="p-6 flex items-center justify-between gap-4 flex-wrap">
                        <div>
                            <p class="font-heading font-medium text-brand-900">{{ $venda->veiculo->marca }} {{ $venda->veiculo->modelo }} {{ $venda->veiculo->ano_modelo }}</p>
                    ...
                    @if ($venda->garantiasChamados->isNotEmpty())
                        <div class="border-t border-brand-100 px-6 py-5">
                            <p class="text-[11px] font-semibold text-muted uppercase tracking-wide mb-3">Chamados de garantia</p>
                            <div class="space-y-3">
                                @foreach ($venda->garantiasChamados as $chamado)
                                    <div class="flex items-start gap-3">
                                        <span class="shrink-0 px-2 py-0.5 rounded-full text-[11px] font-medium bg-brand-100/60 text-brand-700">{{ $chamado->statusLabel() }}</span>
                                        <div>
                                            <p class="text-sm text-ink">{{ $chamado->descricao_problema }}</p>
                                            <p class="text-xs text-muted mt-0.5">Aberto em {{ $chamado->created_at->format('d/m/Y') }}</p>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
        ...
        @if (app('tenant')?->whatsappUrl())
            <div class="mt-8 rounded-2xl border border-brand-100 bg-white p-6">
                <p class="font-heading text-sm font-semibold text-brand-900 mb-1">Precisa acionar a garantia?</p>
                <p class="text-sm text-muted mb-4">Entre em contato com a loja diretamente.</p>
