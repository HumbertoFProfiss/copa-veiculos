<div>
    ...
    <h1 class="font-heading text-2xl font-semibold text-brand-900 dark:text-white mb-1">Minhas Compras</h1>
    <p class="text-sm text-muted dark:text-duskmuted mb-8">Histórico de veículos comprados na loja</p>
    ...
    @if ($compras->isEmpty())
        <div class="rounded-2xl border border-brand-100 dark:border-duskborder bg-white dark:bg-duskcard p-10 text-center">
            ...
            <p class="text-muted dark:text-duskmuted">Nenhuma compra registrada ainda.</p>
        </div>
    @else
        <div class="space-y-4">
            @foreach ($compras as $venda)
                <div class="rounded-2xl border border-brand-100 dark:border-duskborder bg-white dark:bg-duskcard p-6 flex items-center justify-between gap-4 flex-wrap">
                    <div>
                        <p class="font-heading font-medium text-brand-900 dark:text-white">{{ $venda->veiculo->marca }} {{ $venda->veiculo->modelo }}</p>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        @else
                            <span class="px-2.5 py-1 rounded-full text-xs font-medium bg-brand-100/60 dark:bg-duskcard/60 text-muted dark:text-duskmuted">Cancelada</span>
                        @endif
                    </div>
                </div>
            @endforeach
