    ...
    <div class="max-w-7xl mx-auto px-6 sm:px-8 py-16">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div class="lg:col-span-2">
                @if ($veiculo->fotos->isNotEmpty())
                    <div class="relative aspect-[4/3] bg-brand-100/50 rounded-2xl overflow-hidden mb-3 shadow-brand">
                    ...
                    @if ($veiculo->fotos->count() > 1)
                        <div class="grid grid-cols-4 gap-3">
                            @foreach ($veiculo->fotos->skip(1) as $foto)
                                <div class="aspect-[4/3] bg-brand-100/50 rounded-xl overflow-hidden">
                    ...
                    @endif
                @else
                    <div class="aspect-[4/3] bg-brand-100/50 rounded-2xl flex items-center justify-center text-muted">
                        <x-heroicon-o-photo class="w-12 h-12" />
                    </div>
                @endif

                <div class="mt-10">
                    <h2 class="font-heading text-lg font-medium text-brand-900 mb-4">Ficha técnica</h2>
                    <dl class="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
                        <div><dt class="text-muted">Ano</dt><dd class="font-medium text-brand-900">{{ $veiculo->ano_fabricacao }}/{{ $veiculo->ano_modelo }}</dd></div>
                        <div><dt class="text-muted">KM</dt><dd class="font-medium text-brand-900">{{ number_format($veiculo->km, 0, ',', '.') }}</dd></div>
                        <div><dt class="text-muted">Câmbio</dt><dd class="font-medium text-brand-900">{{ $veiculo->cambio ?: '—' }}</dd></div>
                        <div><dt class="text-muted">Combustível</dt><dd class="font-medium text-brand-900">{{ $veiculo->combustivel ?: '—' }}</dd></div>
                        <div><dt class="text-muted">Cor</dt><dd class="font-medium text-brand-900">{{ $veiculo->cor ?: '—' }}</dd></div>
                        <div><dt class="text-muted">Portas</dt><dd class="font-medium text-brand-900">{{ $veiculo->portas ?: '—' }}</dd></div>
                    </dl>
                </div>

                @if ($veiculo->opcionais->isNotEmpty())
                    <div class="mt-10">
                        <h2 class="font-heading text-lg font-medium text-brand-900 mb-4">Opcionais</h2>
                        <div class="flex flex-wrap gap-2">
                            @foreach ($veiculo->opcionais as $opcional)
                                <span class="px-3 py-1.5 rounded-full bg-brand-100/50 border border-brand-100 text-sm text-brand-900">{{ $opcional->nome }}</span>
                            @endforeach
                        </div>
                    </div>
                @endif
            ...
            <div>
                <div class="rounded-2xl border border-brand-100 bg-white p-6 shadow-brand sticky top-24">
                    <h1 class="font-heading text-xl font-semibold text-brand-900">{{ $veiculo->marca }} {{ $veiculo->modelo }}</h1>
                    @if ($veiculo->versao)
                        <p class="text-muted">{{ $veiculo->versao }}</p>
                    @endif
        ...
        @if ($semelhantes->isNotEmpty())
            <div class="mt-20">
                ...
                <h2 class="font-heading text-2xl font-semibold text-brand-900 mb-6">Veículos semelhantes</h2>
